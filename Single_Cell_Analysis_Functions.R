...
# Need Seurat v4.4.0 for now
# Installed remotes package, to use install_version function to get the older v4.4.0 Seurat
library(remotes)
# install_version("Seurat",version = "4.4.0")
library(Seurat)
# Using the command from the github page for it: remotes::install_github("mojaveazure/seurat-disk")
# remotes::install_github("mojaveazure/seurat-disk")
# Have to choose none to not update Seurat to v5.0
library(SeuratDisk)
# Ditto above comments for SeuratWrappers
# remotes::install_github("satijalab/seurat-wrappers")
library(R.utils)
# SeuratWrappers install had non-zero exit status because Seurat v5.0.0 is required
# Went into github and found the SHA for version updated before a commit to require latest version
# remotes::install_github("satijalab/seurat-wrappers@b6d519b69e8a6364f7275d116aa8b0c1ee783364")
...
# pcaMethods needs to be installed with BiocManager
# BiocManager::install("pcaMethods")
# Just as a note, pcaMethods will want to update Seurat v4.4.0, just chose n
library(pcaMethods)
library(hdf5r)
library(harmony)
...
# 'terra', 'ggrastr'))
# Many of these seem to be installed as dependencies of packages above (esp DropletUtils)
# Still need: 
# BiocManager::install(c('lme4', 'batchelor', 'terra', 'ggrastr'))
...
# Installed from github page using a script provided by developers to install URD and dependencies
...
library(URD)
# Matrix is installed by URD command above
library(Matrix)
# Warning from rgl in case it becomes relevant later
# This build of rgl does not include OpenGL functions.  Use
# rglwidget() to display results, e.g. via options(rgl.printRglwidget = TRUE).
library(rgl)

# Needs to be installed with BiocManager, can install with or without dependecnies
...
# For XML did this install in the terminal: conda install r-xml
# Then had to close and reopen R
...
# All of these depend on XML it seems
# Always chose none for updating packages
library(velociraptor)

library(shiny)

# From ShinyCell github:
...
# Also check for Shiny required packages
...
# Adding DESeq2 to automatically loaded packages to do within cluster DE by condition
# Need to install DESeq2 with Bioconductor
# Declined updating other packages during install
# BiocManager::install("DESeq2")
library(DESeq2)

# ggplot already installed as a dependency for something above
library(ggplot2)
library(ggpointdensity)
# viridis already installed by something above
library(viridis)
# Ditto future
library(future)
# Ditto sctransform
...
library(dittoSeq)
# Ditto to stringr already being installed
library(stringr)
# Ditto knitr
library(knitr)
library(clustree)
# Ditto stats, scater, gtools, magrittr, grid, reticulate, dplyr
library(stats)
library(scater)
library(gtools)
library(magrittr)
